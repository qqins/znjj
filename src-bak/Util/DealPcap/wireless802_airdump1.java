 ...
 * //文件头 数据包头+数据报  数据包头 +数据报  .......
  ...
  */
public class wireless802_airdump1 {
...
System.out.print("\t信道类型：802.11b");
	...
	public String frametype(byte by[],int ethloc){
		...
		String bString = wireless802_airdump1.hexString2binaryString(datatype);
...
//System.out.print("	版本:"+version);
	...
	public String duration(byte by[],int ethloc){
		...
		String bString = wireless802_airdump1.hexString2binaryString(duration);
...
//System.out.println("6.fragment num和sequence num 2字节 16bit 当前值:"+sb.toString());
	...
	public String qosControl(byte by[],int ethloc){
		ethloc += 24;
		StringBuilder sb = new StringBuilder();
		for (int m = 0; m < 2; m++) {
			sb.append(Integer.toString((by[ethloc + m] & 0xff) + 0x100, 16).substring(1));
...
//System.out.println("29.tcp windowSize 2字节 16bit 当前值:"+Integer.parseInt(seq.toString(), 16));
	 ...
	 *************************************************/
	public List<String> data_tcp(byte by[],int ethloc,int ipheader,int tcpheader,int len,String qos){
		
		
		//确认数据起始位置
		
		//int dataLen = L+len -ethloc;
		ethloc += 34 + ipheader+tcpheader;
		//确认数据长度	
		int dataLen = 0;
		 dataLen = len - (34 + ipheader+tcpheader);
		if(!qos.equals("0000"))
		 dataLen = len - (34 + ipheader+tcpheader-2);
...
//System.out.println(" ");	
	 ...
	 *************************************************/
	public List<String> data_udp(byte by[],int ethloc,int ipheader,int udplen,String qos){
...
//System.out.println(" ");	
	 ...
	 *************************************************/
	public byte[] data_byte_tcp(byte by[],int ethloc,int ipheader,int tcpheader,int len,String qos){
	 ...
	 *************************************************/
	public byte[] data_byte_udp(byte by[],int ethloc,int ipheader,int udplen,String qos){
...
System.out.print("   radiotap headLen:"+radioHead);		
	...
	public String flags(int ethloc,byte by[]){
		...
		String bString = wireless802_airdump1.hexString2binaryString(ss);
...
//System.out.print(" NOdes 00 ");	
	...
	public static void main(String[] args){
		...
		List<Model_a> ls = w.ergodic("F:\\test_pcap\\ceshi1.pcap");
		int[] index = {2757};
		List<Model_a> index_list = w.getFrameByindex(index,ls);
		
		System.out.println(index_list.get(0).getData_byte().length);
	}
	/*************************************************
	 * end
	 *************************************************/
	...
	/*************************************************
	 * 传入一个int数组，按照传入的顺序，依次拿出对应int值的 帧
	 *************************************************/
	public List<Model_a> getFrameByindex(int[] index,List<Model_a> ls){
		
		List<Model_a> index_list = null;
		if(ls.size() != 0){
			
			index_list = new ArrayList<Model_a>();
			Iterator<Model_a> it = ls.iterator();
			
			int j = 0;
			
			for(Model_a model:ls){
				
				for(int i = 0;i<index.length;i++){
					if(index[i]==model.getIndex()){
						index_list.add(model);
						continue;
					}	
				}
				if(index_list.size() == index.length){	
			...
			/*while(it.hasNext()){
				...
				Model_a model = it.next();
				for(int i = 0;i<index.length;i++){
					if(index[i]==model.getIndex()){
						index_list.add(model);
						System.out.println("aaaaaaaa222:"+index_list.size());
					}		
				}
				
				j++;
				if(index_list.size() == index.length){	
					System.out.println("aaaaaaaa:"+index_list.size());
					break;
				}
			}*/	
	 ...
	 * 根据类型（TCP UDP）来提取帧
	 *************************************************/
	public List<Model_a> getFrameByType(String type,List<Model_a> ls){
		
		List<Model_a> listByType = null;
		if(ls.size() != 0){
			
			listByType = new ArrayList<Model_a>();
			//Iterator<Model_a> it = ls.iterator();
			
			for(Model_a model:ls){
				//System.out.println(model.getnLayer().getNettype());
				if(model.getnLayer().getNettype().equals(type)){
					
					listByType.add(model);
				}
			}
	 ...
	 * List<Model_a> ergodic() 遍历所有的帧 提取TCP和UDP帧 其他帧暂不处理  F:\\test_pcap\\
	 *************************************************/
		public List<Model_a> ergodic(String fileName) {
		 ...
		 * 数据初始化部分
		 *************************************************/
		wireless802_airdump1 wireless802 = new wireless802_airdump1();
		try {
//			if (!file2.exists()) {
//				file2.createNewFile();
//			}
...
//System.out.println("");	
				...
				//在数据帧中，表示MAC地址的有3个字段，但实际中共有5个MAC显示 以下3到5字段是对MAC地址的处理
					...
					//System.out.println(macAddress.substring(1,18));
//System.out.println("");	
				...
				//暂时没有具体处理
					...
					String qosControl = wireless802.qosControl(by, ethloc);
					if(!qosControl.equals("0000"))
						ethloc = ethloc - 2;
					...
					if(len > 25){
						...
						if(nettype.equals("06")){
							...
							if(tcpflag.equals("18")){
								...
								//data 
								dataList = wireless802.data_tcp(by, ethloc, ipHeaderLen, tcpHeaderLen, len,qosControl);
								data_byte = wireless802.data_byte_tcp(by, ethloc, ipHeaderLen, tcpHeaderLen, len,qosControl);
							...
							data_byte = wireless802.data_byte_udp(by, ethloc, ipHeaderLen, udplen,qosControl);
							dataList = wireless802.data_udp(by, ethloc, ipHeaderLen, udplen,qosControl);
						...
						//if(nettype.equals(net) && ipAddress_sou.equals(hostIP) || ipAddress_des.equals(hostIP)){
							...
							wLayer.setDes_mac(macAddress.substring(1, 18));
